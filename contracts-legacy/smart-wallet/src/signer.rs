use smart_wallet_interface::{
    types::{
        Error, LegacySignerVal, Signatures, Signer, SignerExpiration, SignerKey, SignerStorage,
        SignerVal,
    },
    PolicyClient,
};
use soroban_sdk::{auth::Context, panic_with_error, vec, Env, TryFromVal, Val, Vec};
...
/// Decode a stored signer value written by any pre-1.0 build.
///
/// Returns the value plus whether it was stored in the legacy (pre-`6a27d48`)
/// layout. Neither shape decodes as the other, so the order of attempts does
/// not matter for correctness; the current shape is tried first because it is
/// the common case after migration.
fn decode_signer_val(env: &Env, raw: &Val) -> Option<(SignerVal, bool)> {
    if let Ok(val) = SignerVal::try_from_val(env, raw) {
        return Some((val, false));
    }
    if let Ok(legacy) = LegacySignerVal::try_from_val(env, raw) {
        return Some((legacy.into(), true));
    }
    None
}

/// Read a signer entry from one durability, tolerating both layouts.
fn read_signer_val(
    env: &Env,
    signer_key: &SignerKey,
    persistent: bool,
) -> Option<(SignerVal, bool)> {
    let raw: Option<Val> = if persistent {
        env.storage().persistent().get::<SignerKey, Val>(signer_key)
    } else {
        env.storage().temporary().get::<SignerKey, Val>(signer_key)
    };

    match raw {
        None => None,
        Some(raw) => match decode_signer_val(env, &raw) {
            Some(decoded) => Some(decoded),
            // An entry exists under this key but is not a signer value of any
            // known shape. Treat it as corrupt rather than as absent.
...
pub fn get_signer_val_storage(
    ...
    match read_signer_val(env, signer_key, false) {
        Some((signer_val, _)) => {
            if extend_ttl {
        ...
        None => match read_signer_val(env, signer_key, true) {
            Some((signer_val, _)) => {
                if extend_ttl {
...
/// Rewrite a signer entry stored in the legacy layout as a current
/// `SignerVal`, in place, in the same durability. Returns `true` when a
/// rewrite happened. The decoded value is identical either way, so this is a
/// pure re-encoding: no expiration, limits, public key, or durability changes.
pub fn normalize_signer(env: &Env, signer_key: &SignerKey) -> bool {
    for persistent in [false, true] {
        if let Some((signer_val, was_legacy)) = read_signer_val(env, signer_key, persistent) {
            if was_legacy {
                if persistent {
                    env.storage()
                        .persistent()
                ...
                extend_signer_key(env, signer_key, persistent);
            }
            return was_legacy;
        }
    }
